...
${truncatedContent}`;
  ...
  async assistRefineRecommendation(
    ...
    try {
      ...
      // Fetch LLM config
      console.log(`[LLMAssistanceService] Fetching LLM config for recommendation refinement, app: ${applicationId}`);
      const llmConfig = await configManager.getApplicationLLMConfig(applicationId);
      ...
      // Build refinement prompt
      const systemPrompt = `You are an expert technical writer and recommendation specialist. Your task is to improve and refine recommendation text.

When refining recommendations:
1. Improve clarity and conciseness
2. Make the language more professional and precise
3. Ensure actionable next steps are clear
4. Maintain the original intent and key information
5. Fix any grammatical or spelling issues
6. Add specificity where vague

Return ONLY the refined recommendation text, no explanations or metadata.`;

      const userPrompt = `Please refine and improve this recommendation:

${recommendationText}`;

      console.log(`[LLMAssistanceService] Generating refined recommendation using ${llmConfig.provider}`);
  ...
  /**
   * Assist in generating summaries and metadata for knowledge base content
   * 
   * @param applicationId - Application identifier
   * @param contentText - KB document content to summarize
   * @returns LLM suggestion for content summary
   * @throws Error if LLM config missing or generation fails
   */
  async assistGenerateKBSummary(
    applicationId: string,
    ...
    try {
      ...
      // Fetch KB LLM config (different from dashboard LLM)
      console.log(`[LLMAssistanceService] Fetching KB LLM config for summary generation, app: ${applicationId}`);
      const kbConfig = await configManager.getApplicationKBConfig(applicationId);

      // Validate config
      const validation = configManager.validateKBConfig(kbConfig);
      if (!validation.valid) {
        throw new Error(`Invalid KB LLM config: ${validation.errors.join(', ')}`);
      }

      // Create LLM client using KB provider
      const llmClient = LLMClientFactory.create(kbConfig);

      // Validate connection
      const connectionTest = await llmClient.validate();
      if (!connectionTest.valid) {
        throw new Error(`KB LLM connection validation failed: ${connectionTest.error}`);
      }

      // Build summary generation prompt
      const systemPrompt = `You are an expert knowledge management specialist. Your task is to generate concise, accurate summaries of technical content.

When generating summaries:
1. Capture the key concepts and main ideas
2. Include important definitions and terminology
3. Highlight critical information and next steps
4. Keep language clear and professional
5. Make the summary 15-20% of the original content length
...
Return ONLY the summary text, no explanations or metadata.`;
      ...
      const truncatedContent = contentText.length > 10000 
        ? contentText.substring(0, 10000) + '...[content truncated]'
        : contentText;

      const userPrompt = `Please generate a comprehensive summary of this knowledge base content:

${truncatedContent}`;

      console.log(`[LLMAssistanceService] Generating KB summary using configured KB provider`);

      // Generate suggestion
      const suggestion = await llmClient.generate(userPrompt, {
        temperature: 0.3,
        maxTokens: kbConfig.maxTokens ?? 1024,
        systemPrompt,
      });
