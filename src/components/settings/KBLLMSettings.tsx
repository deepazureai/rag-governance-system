...
export const KBLLMSettings: React.FC<KBLLMSettingsProps> = ({ applicationId: initialAppId }) => {
  ...
  // Load existing config when selected app changes
  useEffect(() => {
    if (selectedAppId) {
      loadConfig();
    }
  ...
  const handleTestConnection = async (): Promise<void> => {
    ...
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001';
      const response = await fetch(`${apiUrl}/api/kb-config/validate/${selectedAppId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          provider,
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-white">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Configure KB LLM credentials for individual applications. Each application&apos;s configuration is isolated for cost tracking.
        </p>
      </Card>
      ...
      <Card className="p-6 bg-white">
        <div className="flex items-center gap-2 mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Knowledge Base LLM Provider Configuration</h3>
          {provider && <Badge className="bg-blue-100 text-blue-800">{provider}</Badge>}
        </div>
